...
class upgrade_calendar_event_uuids_task_test extends advanced_testcase {
    ...
    /**
     * Sets the {@see hostname} property to the `$CFG->wwwroot` value with the scheme stripped.
     */
    protected function setUp(): void {
        global $CFG;
        parent::setUp();
        $this->hostname = preg_replace('|https?://|', '', $CFG->wwwroot);
    }
    ...
    public function test_clean_up_recursive_events_self_subscription(): void {
        ...
        ob_start();
        $numdeleted = $task->clean_up_recursive_events($this->hostname);
        $output = ob_get_clean();
