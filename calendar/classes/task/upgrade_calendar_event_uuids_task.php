...
class upgrade_calendar_event_uuids_task extends adhoc_task {
     ...
     */
    public static function instance(string|null $hostname = null): self {
        $task = new self();
        $task->set_custom_data(['hostname' => $hostname]);
        return $task;
    }
    ...
    public function clean_up_recursive_events(string $hostname): int {
        ...
        $getparentevent = function (object $event) use ($DB, $parentsql, $params): object|false {
            if (!$parentid = self::get_parent_event_id($event->uuid)) {
                return false;
            }
