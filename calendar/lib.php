...
/**
 * Return the host used in the UID of a Moodle calendar.
 *
 * @return string the webroot without `http://` or `https://`
 */
function calender_get_host() : string {
    global $CFG;
...
function calendar_add_icalendar_event($event, $unused, $subscriptionid, $timezone='UTC') {
    ...
    if (preg_match('/(\d+)@(.+)/', $eventrecord->uuid, $matches)) {
        // The UID of the event looks like it was generated by a Moodle calendart export.
        list(, $id, $host) = $matches;
        if ($host == calender_get_host() && $DB->get_record('event', ['id' => $id, 'subscriptionid' => $subscriptionid])) {
            // The event was already imported and associated with this subscription.
